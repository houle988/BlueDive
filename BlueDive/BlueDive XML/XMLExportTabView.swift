...
// MARK: - XML Export Tab View

/// Displays a formatted XML representation of a single dive's internal data.
/// XML generation is handled by BlueDiveXMLExporter.
struct XMLExportTabView: View {
    ...
    var body: some View {
        ...
        .onAppear { xmlString = BlueDiveXMLExporter.generateXML(for: dive) }
        #if os(iOS)
        .fileExporter(
    ...
    private var headerSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Structure: BlueDiveGlobalData · BlueDiveSiteData · BlueDiveTankData · BlueDiveMarineLifeData · BlueDiveGearData · BlueDiveSamplesData")
                .font(.caption2)
            ...
            Button {
                ...
                .background(
                    RoundedRectangle(cornerRadius: 8)
                        .fill((isCopied ? Color.green : Color.indigo).opacity(0.15))
                )
            }
            .buttonStyle(.plain)
    ...
    private var xmlPreviewSection: some View {
        let allLines = xmlString.components(separatedBy: "\n")
